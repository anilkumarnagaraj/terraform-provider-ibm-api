...
func TerraformerImport(configDir, resources string, scenario string, timeout *time.Duration, randomID string) error {
	...
	return run("terraformer", []string{"import", "ibm", fmt.Sprintf("--resources=%s", resources), "--compact", fmt.Sprintf("-p=%s/%s", configDir, "discovery")}, configDir, scenario, timeout, randomID)
}
...
// MergeStateFile ..
func MergeStateFile(terraformfObj, terraformerObj ResourceList, src, dest, configDir, scenario, randomID string, timeout *time.Duration) {
	var isExist bool
